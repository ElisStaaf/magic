...
(define-test-case "magic common test"
  ("get-param  test"
   (let ((params `(("int" 1)
                   ("string" "string")
                   ("symbol" sym)
   ...
   ("get-value/set-value! test"
    (parameterize ((session (make-magic-session)))
      ...
      (assert-each-value (lambda (key value)
                         ...
                         `((int 1)
                           ...
                           (false #f)))))
   ("get-id/set-id! test"
    (assert-equal #f (get-id (make-magic-session)))
    (let ((ids '(1 0 -1 :a a)))
      (assert-each (lambda (id)
                     (let ((sess (make-magic-session)))
                       (set-id! sess id)
                       (list id (get-id sess))))
                   ids)
      (let ((sess (make-magic-session)))
   ...
   ("get-action test"
    ...
    (let ((key (x->string *magic-action-key*)))
