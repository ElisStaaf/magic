...
(define-test-case "magic session test"
  ("make-magic-session test"
   (assert-each-list-elem
    (lambda (key&values . args)
      (list (apply make-magic-session args)
            key&values))
   ...
   (assert-each (lambda args
                  (assert-error (lambda () (apply make-magic-session args))))
                '((:one-keyword)
                  ...
                  (let ((s (make-magic-session)))
                    (for-each (lambda (elem)
                                (apply (lambda (key value)
                                       ...
                                       elem))
                              (slices args 2))
                    (assert-equal (sort (cons (list *magic-id-key* (get-id s))
                                              (slices expected 2))
                                        compare)
  ...
  ("valid? test"
   (assert-true (valid? (make-magic-session #f)))
   (assert-false (valid? (make-magic-session 0)))
   (assert-true (valid? (make-magic-session 30))))
  )
